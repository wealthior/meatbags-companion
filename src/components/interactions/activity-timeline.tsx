...
const TX_CONFIG: Record<TransactionType, {
  ...
  BURN: { icon: Flame, label: "Burn", color: "text-blood", bgColor: "bg-blood/10" },
};

export function ActivityTimeline({ transactions, maxItems = 50 }: ActivityTimelineProps) {
  const visible = transactions.slice(0, maxItems);

  if (visible.length === 0) {
    return (
      <div className="text-center py-8 text-text-muted text-xs">
        No activity recorded in the wasteland.
      </div>
    );
  }
